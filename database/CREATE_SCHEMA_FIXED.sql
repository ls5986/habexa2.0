-- ============================================
-- HABEXA DATABASE SCHEMA - CREATE ONLY (FIXED)
-- ============================================
...
-- Note: auth.users is a Supabase system table that should already exist
...
-- ============================================
-- PART 7: INDEXES (with IF NOT EXISTS)
-- ============================================

-- Core indexes
CREATE INDEX IF NOT EXISTS idx_deals_user_status ON public.deals(user_id, status);
CREATE INDEX IF NOT EXISTS idx_deals_asin ON public.deals(asin);
...
CREATE INDEX IF NOT EXISTS idx_analyses_asin ON public.analyses(asin);
...
CREATE INDEX IF NOT EXISTS idx_telegram_messages_unprocessed ON public.telegram_messages(user_id, is_processed) WHERE is_processed = FALSE;
...
CREATE INDEX IF NOT EXISTS idx_telegram_deals_pending ON public.telegram_deals(user_id, status) WHERE status = 'pending';
